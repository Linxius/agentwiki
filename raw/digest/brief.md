...
- 来源: https://arxiv.org/abs/2604.01204
- 源文件: [raw/digest/sources/2026-07-29/arxiv-260401204-6a516cc9.md](/raw/digest/sources/2026-07-29/arxiv-260401204-6a516cc9.md)
- 标题: 神经谐波纹理：面向高质量基元式神经重建
- 领域: 3D重建/计算机图形学
- 关键词: 3D高斯泼溅, 神经辐射场, 实时渲染, 显式表示, 新视角合成
- 匹配: 3D高斯泼溅
- 理由: 论文直接研究 3D 高斯泼溅（3DGS）等 primitive-based 重建方法，提出 Neural Harmonic Textures 提升单个基元的表达能力，并可无缝集成进 3DGUT、Triangle Splatting、2DGS 等现有 3DGS 管线，在实时新视角合成上达到 SOTA。
- [ ] 深度阅读
- [ ] 合入 wiki
- [ ] 不感兴趣
![Neural Harmonic Textures 方法概览：在基元外围虚拟支架锚定特征，于光线交点插值并经周期激活后沿射线谐波合成，最后由轻量 MLP 延迟解码](https://arxiv.org/html/2604.01204v3/x1.png)
**Neural Harmonic Textures 方法概览：在基元外围虚拟支架锚定特征，于光线交点插值并经周期激活后沿射线谐波合成，最后由轻量 MLP 延迟解码**
Neural Harmonic Textures 方法概览：在基元外围虚拟支架锚定特征，于光线交点插值并经周期激活后沿射线谐波合成，最后由轻量 MLP 延迟解码。

**简介**：针对 3D 高斯泼溅等基元式方法单个基元表达力不足、需用大量基元刻画高频细节的问题，提出 Neural Harmonic Textures：在每个基元外围的虚拟支架上锚定可学习特征，在光线交点处插值并用正弦/余弦周期激活，将 alpha 混合转化为谐波分量的加权和，最后用一个小 MLP 在延迟着色阶段单次解码出像素颜色。该表示与基元类型无关，可无缝接入 3DGUT、Triangle Splatting、2DGS 等现有管线，在实时与离线新视角合成基准上均取得 SOTA，并扩展到语义重建。

**详细报告**（主要思路与方法流程）：
要解决的问题：3D 高斯泼溅等 Lagrangian 基元式方法虽渲染快、易编辑，但单个基元表达能力有限，高频空间细节和镜面高光被迫靠堆砌大量基元来刻画，带来显存与速度的双重代价。

核心思路：简单来说就是把基元既当几何载体、又当“局部位置编码”。作者在每个基元（高斯或三角形）外包一层虚拟支架，锚定若干可学习特征向量；光线打到基元时，在交点处对这些特征做局部插值。

方法流程：1）对插值后的特征施加正弦/余弦周期激活，灵感来自傅里叶分析——激活后的特征相当于频率分量，基元的不透明度则调制其振幅；2）沿光线做 alpha 混合时，不再是混合 RGB，而是混合这些谐波分量，得到图像空间里足够丰富的隐式信号；3）最后用一个小 MLP 对每个像素一次性解码（延迟着色），避免了逐基元多次神经网络求值。因特征相对基元锚定，运动、形变、编辑天然得到支持。

效果/实验：在 standard 新视角合成基准上，实时与离线方法均达到 SOTA；且该公式对基元类型无关（三角形、2D/3D 高斯、四面体均可），可即插即用到现有 3DGS 管线。论文还展示其在 2D 图像拟合与语义重建上的通用性。

局限/潜在问题：表达力提升依赖 MLP 解码的额外计算与训练复杂度，相较纯 3DGS 的解析着色会引入推理开销；特征的插值与外推在极端视角或稀薄基元区域下的稳定性仍需验证。

## [不感兴趣]

#### MASS: Motion-Aligned Selective Scan for Refinement in Flow-Based Video Frame Interpolation
- 来源: https://arxiv.org/abs/2606.27718
- 源文件: [raw/digest/sources/2026-07-29/arxiv-260627718-3ff0bce9.md](/raw/digest/sources/2026-07-29/arxiv-260627718-3ff0bce9.md)
- 标题: MASS：面向流基视频帧插值的运动对齐选择性扫描
- 领域: 视频处理/计算机视觉
- 关键词: 视频帧插值, 状态空间模型, 光流, 运动建模
- 匹配: 无
- 理由: 论文研究视频帧插值（VFI），基于选择性状态空间模型（SSM/Mamba）将特征扫描顺序从静态网格改为沿光流轨迹，属于视频处理/时序建模方向，与兴趣列表中的 3D 高斯泼溅、神经重建无直接关联。
- [ ] 深度阅读
- [ ] 合入 wiki
- [ ] 不感兴趣
![MASS 特征序列化策略对比：(a) 静态光栅扫描将错位网格压平成混合序列，产生拖影；(b) MASS 沿代理轨迹提取特征形成运动一致序列，降低动态区域错位](https://arxiv.org/html/2606.27718v1/fig_1.png)
**MASS 特征序列化策略对比：(a) 静态光栅扫描将错位网格压平成混合序列，产生拖影；(b) MASS 沿代理轨迹提取特征形成运动一致序列，降低动态区域错位**
MASS 特征序列化策略对比：(a) 静态光栅扫描将错位网格压平成混合序列，产生拖影；(b) MASS 沿代理轨迹提取特征形成运动一致序列，降低动态区域错位。

**简介**：针对视频帧插值中基于选择性状态空间模型（SSM）的方法受静态网格扫描限制、与物理运动错位的问题，提出 Motion-Aligned Selective Scan（MASS）：沿每个像素的光流引导轨迹构建特征序列并用 SSM 聚合，配合可学习非线性路径积分与按运动幅度自适应采样步长，再由聚合状态引导修正中间光流与遮挡掩码。在标准基准上取得有竞争力的结果，在大位移与复杂动态场景下表现突出。

**详细报告**（主要思路与方法流程）：
要解决的问题：视频帧插值（VFI）在大幅非线性运动与遮挡下，中间光流难以准确估计；现有 SSM（如 Mamba）类方法多在静态空间网格上序列化特征，与目标像素沿物理运动路径最相关的上下文错位。

核心思路：简单来说就是把“按像素网格扫描”换成“按运动轨迹扫描”。既然目标像素最有用的信息在它的运动路径上，就沿光流构造代理轨迹，把特征拉成一条一维序列再做状态空间建模。

方法流程：1）给定粗中间光流，为每个像素构造 flow-guided 代理轨迹；2）沿轨迹采样特征并展平为一维序列输入 SSM 聚合，避免运动不一致邻域的干扰；3）引入可学习残差更新轨迹状态以逼近弯曲路径，并用 velocity-aware SSM 按位移大小动态分配采样密度（快速运动更密、静态区域更省）；4）聚合状态进一步引导一个修正模块端到端地纠正中间光流与掩码。

效果/实验：在多个标准 VFI 基准上取得有竞争力、部分 SOTA 的结果，尤其在大位移与遮挡的复杂动态场景中鲁棒性明显更优，且不显著增加计算开销。

局限/潜在问题：方法依赖粗光流估计的质量，光流完全失效的区域（如无纹理或大遮挡）轨迹构造仍会退化；引入可学习路径与自适应长度也增加了训练与调参复杂度。
